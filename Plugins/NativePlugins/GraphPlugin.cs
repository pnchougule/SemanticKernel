...
namespace SemanticKernalTest.Plugins.NativePlugins
{
    public class GraphPlugin
    {
        ...
        {
            var graphApiToken = RetrieveFromCache("GraphToken");
            if (string.IsNullOrEmpty(graphApiToken))
            {
                var auth = new Auth();
                var token = auth.GetAccessToken();
                TokenManager.AccessToken = token;
                StoreInCache("GraphToken", token);
            }
            else
            {
                TokenManager.AccessToken = RetrieveFromCache("GraphToken");
            }
        }
        ...
        public static dynamic RetrieveFromCache(string key)
        {
       ...
       userQuer: compose an email to balajimogadali@gmail.com congratulating him on his new milestone.
       
       Assistant Response: Congratulations Balaji!!!! may you achieve more milestones like this!!!";
